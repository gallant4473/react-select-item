...
export default class SelectItem extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      searchText: "",
      filterConstrain: this.props.filterConstrain? this.props.filterConstrain: -1,
      restrict: false
    };
  }
    ...
    return (event) => {
      ...
      if (this.isMultiple()) {
        ...
        if (val !== null) {
          ...
          if (index !== -1) {
          ...
          } else if(this.state.filterConstrain > selected.length || this.state.filterConstrain== -1) {
            selected.push(val)
            this.setState({
              restrict: false
            })
            ...
            this.props.filterConstrainFeedback()
          }
        }
        this.updatePendingValue(selected, cb) || this.props.onChange(selected, this.state.restrict);
        if (this.state.searchEnabled) {
          this.setState({ open: true })
        }

      } else {
        this.updatePendingValue(val, cb) || this.props.onChange(val, this.state.restrict);
        this.handleClose();
        if (!this.state.searchEnabled) {
  ...
  handleNativeChange = (event) => {
    ...
    this.props.onChange(val, this.state.restrict)
  };
