  ...
  handleChange = (val, cb) => {
    return (event) => {
      ...
      if (this.isMultiple()) {
        ...
        this.updatePendingValue(selected, cb) || this.props.onChange(selected, !(this.state.filterConstrain > selected.length-1 || this.state.filterConstrain== -1));
        if (this.state.searchEnabled) {
          this.setState({ open: true })
        }

      } else {
        this.updatePendingValue(val, cb) || this.props.onChange(val, false);
        this.handleClose();
        if (!this.state.searchEnabled) {
